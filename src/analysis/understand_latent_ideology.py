...
n = nx.from_pandas_edgelist(df_filtered, source='source', target='target', edge_attr='weight', create_using=nx.DiGraph())
...
# %%
df_scores_target.rename(columns={'target':'user'}, inplace=True)
df_scores_source.rename(columns={'source':'user'}, inplace=True)
df = pd.concat([df_scores_target, df_scores_source], ignore_index=True)
df.set_index('user', inplace=True)

# add score to network
nx.set_node_attributes(n, df['score'].to_dict(), 'score')

# %%
pos = {n: [d['score'] + np.random.normal(0, 0.01), np.random.normal(0, 0.01)] for n, d in n.nodes(data=True)}

#i,j,k,l,m bigger node 
size = [1000 if node in ['i','j','k','l','m'] else 100 for node in n]


# %%
nx.draw(n, pos=pos , with_labels=True, width=0.3, node_size=size, node_color=colors)
